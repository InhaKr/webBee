...
function addProduct(e) {
    ...
    if (e.target.classList.contains('product-wishlist') && e.target.getAttribute('stroke') === 'grey') {
        document.querySelectorAll(`[data-id="${e.target.dataset.id}"]`).forEach(function(element) {
            element.setAttribute('stroke', element.getAttribute('stroke') === 'grey' ? 'red' : 'grey');
        });
        count += 1;
        console.log(count);
    ...
    if (e.target.classList.contains('delete')) {
        ...
        document.querySelectorAll(`[data-id="${number}"]`).forEach(function(el) {
            el.setAttribute('stroke', el.getAttribute('stroke') === 'grey' ? 'red' : 'grey');
        });
        r.remove();
    }
...
//# sourceMappingURL=webBee.8ec8d9d5.js.map
